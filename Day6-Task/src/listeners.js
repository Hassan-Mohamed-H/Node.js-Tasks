...
bus.on("order:packed", ({ id }) => {
    ...
    if (order.status !== "paid") {
        return bus.emit("error", new Error("Pack requires status=paid"));
    }
...
bus.on("order:shipped", ({ id }) => {
    ...
    if (order.status !== "packed") {
        return bus.emit("error", new Error("Ship requires status=packed"));
    }
...
bus.on("order:canceled", ({ id }) => {
    ...
    if (order.status === "shipped") {
        return bus.emit("error", new Error("Cannot cancel shipped order"));
    }
